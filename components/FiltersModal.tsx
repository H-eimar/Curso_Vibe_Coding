...
export default function FiltersModal({ isOpen, onClose }: FiltersModalProps) {
  ...
  return (
    <>
      ...
      {/* Modal Overlay */}
      <div 
        className={`fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />

      {/* Main Modal Container */}
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none transition-all duration-300 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        <main className={`relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
          {/* Header */}
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select className="w-full bg-[#f5f8f6] dark:bg-gray-800 border-0 rounded-lg py-3 pl-4 pr-10 text-gray-900 dark:text-white appearance-none focus:ring-2 focus:ring-mosque cursor-pointer">
